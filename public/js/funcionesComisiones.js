const url = "https://cosmeticback.onrender.com/comisiones"
    ...
    .then(function (data) {
      ...
      datos = listaComisiones.map(function (comision) {
        respuesta += `<tr><td>${comision._id}</td>` +
          ...
          <a class='btn btn-danger' onclick='eliminar("${comision._id}")'><i class="fa-solid fa-trash-can"></i></a></td></tr>`
...
const obtenerEmpleados= async () => {
  try {
    const response = await fetch("https://cosmeticback.onrender.com/empleados", {
      method: "GET",
      mode: "cors",
      headers: { "Content-type": "application/json; charset=UTF-8" },
    });
