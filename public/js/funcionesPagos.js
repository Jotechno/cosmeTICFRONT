const url = "https://cosmeticback.onrender.com/pagos"
    ...
    .then(function (data) {
      ...
      datos = listaPagos.map(function (pago) {
        respuesta += `<tr><td>${pago._id}</td>` +
          ...
          `<td>
          <a class='btn btn-primary' onclick='editar(${JSON.stringify(pago)})'><i class="fa-solid fa-pen-to-square"></i></a>
          <a class='btn btn-danger' onclick='eliminar("${pago._id}")'><i class="fa-solid fa-trash-can"></i></a></td></tr>`
...
const obtenerClientes = async () => {
  try {
    const response = await fetch("https://cosmeticback.onrender.com/clientes", {
      method: "GET",
      mode: "cors",
      headers: { "Content-type": "application/json; charset=UTF-8" },
    });
...
const obtenerFactura = async () => {
  try {
    const response = await fetch("https://cosmeticback.onrender.com/ventas", {
      method: "GET",
      mode: "cors",
      headers: { "Content-type": "application/json; charset=UTF-8" },
    });
